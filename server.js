...
// Serve QR as actual PNG image — browser just uses <img src="/api/sessions/p1/qr.png">
...
app.delete('/api/sessions/:id',async(req,res)=>{const sid=req.params.id,s=sessions[sid];
  try{
    ...
    // Remove communities that belonged to this phone
    let removed=0;
    ...
    // Remove session folder from disk so it doesn't auto-reconnect on restart
...
app.get('/api/communities',(req,res)=>{
  ...
  // Merge phone info from both db.phones and live sessions
  const phoneMap={};
...
app.get('/api/group/:groupId/members',async(req,res)=>{const{groupId}=req.params;const s=findSessionForGroup(groupId);if(!s?.sock)return res.status(503).json({error:'No session'});
  try{
    ...
    // Debug: log first participant to see available fields
    ...
    const members=await Promise.all(m.participants.map(async(p)=>{
      ...
      // Try multiple ways to get the real phone number
      let realNumber=null;

      // Method 1: phoneNumber field on participant (Baileys v7)
      if(p.phoneNumber){
      ...
      // Method 5: if id is already a phone number (not LID)
      if(!isLid){
        realNumber=lidNum;
      }
...
app.post('/api/config',(req,res)=>{
  if(req.body.alertNumbers){
    CONFIG.ALERT_NUMBERS=req.body.alertNumbers.split(',').map(n=>n.trim().replace(/[^0-9]/g,'')).filter(Boolean);
  }else if(req.body.waNumber){
    // Legacy single number support
...
app.get('/api/config',(req,res)=>{
  res.json({alertNumbers:CONFIG.ALERT_NUMBERS,warnLimit:CONFIG.WARN_LIMIT,maxLimit:CONFIG.MAX_LIMIT});
});
